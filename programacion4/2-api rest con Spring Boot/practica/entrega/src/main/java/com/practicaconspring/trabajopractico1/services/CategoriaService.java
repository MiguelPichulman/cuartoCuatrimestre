...
//@transactional ???
    public CategoriaDto updateCategoria(Long id, CategoriaEdit dto) {
        Categoria categoria = categoriaRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Error: Categoría no encontrada con id: " + id));

        dto.applyTo(categoria);

        Categoria guardada = categoriaRepository.save(categoria);
        return CategoriaDto.toDto(guardada);
    }
